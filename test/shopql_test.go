...
func TestApp(t *testing.T) {
	...
	testCases := []*ApiTestCase{
		...
		&ApiTestCase{
			Name: "Item's rate again by same user",
			GQL: `
			mutation {
				RateItem(in: {itemID: 1, rate: 3}) {
					id
					name
					rate
				}
		...
		&ApiTestCase{
			Name: "Item's rate by second user",
			GQL: `
			mutation {
				RateItem(in: {itemID: 1, rate: 1}) {
					id
		...
		//---------------------------------------------------------------------
		&ApiTestCase{
			Name: "Comment to item",
			GQL: `
			mutation {
				AddCommentToItem(in: {itemID: 1, commentText: "dqdqdq"}) {
					commentText,
					rate
				}
			}
		...
		&ApiTestCase{
			...
			GQL: `
			mutation {
				AddCommentToComment(in: {commentID: "68a0eb6d88885af37eb5c165", commentText: "dqdqdq"}) {
					commentText,
					itemsID,
					rate
				}
		...
		&ApiTestCase{
			...
			URL:            "/login",
			Method:         http.MethodPost,
			BodyRaw:        "{\"user\":{\"email\":\"admin@example.com\", \"password\":\"123\"}}",
			ResponseStatus: 200,
			CheckFunc: func(resp interface{}) error {
				val, err := lookup.LookupString(resp, "body.token")
				if err != nil {
				...
				fmt.Println("-------------------- TOKEN:", val)
				tplParams["AdminToken"] = val.String()
				return nil
			},
		},
		&ApiTestCase{
			Name: "Add item by admin",
			GQL: `
			mutation {
				AddItem(in: {itemID: 14, catalogID: 1, name: "Smth to rad", sellerID: 1, inStock: 12}) 
				{
					id,
					name,
				}
		...
		&ApiTestCase{
			Name: "Add item by user",
			GQL: `
			mutation {
				AddItem(in: {itemID: 15, catalogID: 1, name: "Smth to rad", sellerID: 1, inStock: 12})
				{
					id,
					name,
				}
		...
		&ApiTestCase{
			...
			GQL: `
			mutation {
				AddCatalog(in: {catalogID: 6, name: "Verden"}) 
				{
					id,
					name,
				}
		...
		// ----------------------------------------------------------------------------------------
		&ApiTestCase{
			Name: "Add to cart - second item",
			GQL: `
					mutation {
						AddToCart(in: {itemID: 5, quantity: 1}) {
						  item {
							id
							name
							inStockText
		...
		&ApiTestCase{
			...
			{"data":
				{"CreateAnOrder":
					{"orderID":1,
					"items":[
					{"quantity":4,"item":{"id":12,"name":"Да Хун Пао", "in_stock": 5}},
					{"item": {"id":5, "in_stock":1, "name":"Язык программирования Go | Донован Алан А. А., Керниган Брайан У."}, "quantity":1}
					],
				...
				"MyOrders":[
						{
							"orderID":1,
							"items":
							[{"quantity":4,"item":{"name":"Да Хун Пао"}},
							{"quantity":1,"item":{"name":"Язык программирования Go | Донован Алан А. А., Керниган Брайан У."}}]
		...
		&ApiTestCase{
			Name: "Check users orders by not admin",
			GQL: `
			query {
				UserOrders(ID: 1){
					orderID,
			...
			ExpectedRaw: `
			{
				"errors":[{"message":"Forbiden","path":["UserOrders"]}],
				"data":null
			}
			`,
		},
	...
	for _, item := range testCases {
		ok := t.Run(item.Name, func(t *testing.T) {
			...
			if item.BodyRaw != "" {
				username = username + "1"
				tplParams["USERNAME"] = username
				tplParams["EMAIL"] = username + "@example.com"
	...
	dbh := &dbutils.SqlDbHandler{Db: Db}
	...
	dbh1 := &dbutils.MongoDbHandler{Db: client.Database("hz")}
	err = dbh.DeleteAllFromTable("users")
	if err != nil {
		log.Println(err.Error())
	}
	dbh.DeleteAllFromTable("sessions")
	dbh1.DeleteFromCollection("Carts")
	dbh1.DeleteFromCollection("Rates")
	dbh1.DeleteFromCollection("Items")
	if err != nil {
		log.Println(err)
	}
	if err != nil {
